...
"""
GenesisChronicles Battle Engine v2.0.0 — ATC-9000 Standard
...
class BattleEngine:
    ...
    def _damage(
        self, attacker: BattleGenesisChronicles, defender: BattleGenesisChronicles, move: Move
        ...
        base = (
            (2 * attacker.level / 5 + 2) * move.power * attacker.attack / defender.defense
        ) / 50 + 2
    ...
    def _priority(
        self, a: BattleGenesisChronicles, b: BattleGenesisChronicles
    ) -> Tuple[BattleGenesisChronicles, BattleGenesisChronicles]:
        """Schnelleres GenesisChronicles greift zuerst an."""
        if a.speed >= b.speed:
            return a, b
        return b, a

    def battle(
        self, s1: BattleGenesisChronicles, s2: BattleGenesisChronicles, max_rounds: int = 50
    ) -> dict:
        ...
        while s1.is_alive() and s2.is_alive() and round_num < max_rounds:
            ...
            for att, dfn in [(first, second), (second, first)]:
                ...
                event = ""
                if eff > 1:
                    event = "super effective!"
                elif eff < 1 and eff > 0:
                    event = "not very effective..."
                if crit:
                    event += " Critical hit!"
                rounds.append(
                    BattleRound(
