...
# GenesisChronicles NFT Contract — ATC-9000 Standard
#
# GenesisChronicles sind NFT-Battle-Kreaturen im A-TownChain Ökosystem.
# Jedes GenesisChronicles ist einzigartig (NFT) mit:
...
class GenesisChroniclesNFT:
    """Ein einzelnes GenesisChronicles NFT."""
...
class GenesisChroniclesContract:
    """
    ATC-9000 Smart Contract — GenesisChronicles NFT Registry.
    Verwaltet Mint, Transfer, Battle, Breeding.
    """

    # Bekannte GenesisChronicles-Namen nach Element
    NAMES = {
    ...
    # ── Minting ────────────────────────────────────────
    def mint(
        self, owner: str, element: str = None, rarity: str = None, generation: int = 1
    ) -> dict:
        ...
        # Name
        el_key = el_name
        names = self.NAMES.get(el_key, ["GenesisChronicles"])
        name = random.choice(names) + f"-{self.total_minted + 1:04d}"

        # Token ID
        token_id = (
            "SHV-" + hashlib.sha256(f"{owner}{name}{time.time()}".encode()).hexdigest()[:12].upper()
        )
    ...
    def battle(self, attacker_id: str, defender_id: str) -> dict:
        ...
        for rnd in range(1, 6):
            ...
            d_hp -= dmg_a
            rounds.append(
                {"round": rnd, "attacker": a.name, "damage": dmg_a, "defender_hp": max(0, d_hp)}
            )
            if d_hp <= 0:
                break
            ...
            a_hp -= dmg_d
            rounds.append(
                {"round": rnd, "attacker": d.name, "damage": dmg_d, "defender_hp": max(0, a_hp)}
            )
            if a_hp <= 0:
                break
